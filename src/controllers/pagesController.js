...
const showCommunityPage = async (req, res) => {
  try {
    ...
    logger.error('Erro ao carregar página de comunidade:', error);
    res.render('pages/community', {
      title: 'Comunidade',
      partnerStores: []
    });
...
const submitPartnerStoreApplication = async (req, res) => {
  try {
    const userId = req.session?.user?.id;
    if (!userId) return res.status(401).json({ message: 'Fa�a login para enviar o formul�rio.' });
    ...
    if (!storeName || String(storeName).trim().length < 2) {
      return res.status(400).json({ message: 'Nome da loja � obrigat�rio.' });
    }
    ...
    if (!created) {
      if (record.status === 'APPROVED') {
        return res.status(400).json({ message: 'Sua loja j� est� aprovada.' });
      }
      await record.update({
        storeName: String(storeName).trim(),
        logoUrl: logoUrl || null,
    ...
    res.json({ message: 'Solicita��o enviada. Vamos analisar e avisar voc�.' });
  } catch (error) {
    logger.error('Erro ao enviar solicita��o de loja parceira:', error);
    res.status(500).json({ message: 'Erro ao enviar solicita��o.' });
  }
};
